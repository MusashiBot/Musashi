...
const keyword_matcher_1 = require("../src/analysis/keyword-matcher");
const mock_markets_1 = require("../src/data/mock-markets");
const fs = require('fs');
...
function parseArgs(argv) {
    ...
    if (!Number.isFinite(opts.threshold) || opts.threshold < 0 || opts.threshold > 1)
        usage();
    if (!Number.isInteger(opts.maxResults) || opts.maxResults < 1)
        usage();
    if (!Number.isInteger(opts.k) || opts.k < 1)
        usage();
    if (!Number.isInteger(opts.showErrors) || opts.showErrors < 0)
        usage();
    return opts;
}
...
function loadMarkets(filePath) {
    if (!filePath)
        return mock_markets_1.mockMarkets;
...
function main() {
    ...
    const matcher = new keyword_matcher_1.KeywordMatcher(markets, opts.threshold, opts.maxResults);
    let tp = 0;
    let fp = 0;
    let fn = 0;
    let tn = 0;
    ...
    for (let i = 0; i < cases.length; i++) {
        ...
        latenciesMs.push(Number(elapsedNs) / 1000000);
        ...
        sumPAtK += pAtK;
        sumRAtK += rAtK;
        if (matches.length > 0 && relevant.has(matches[0].market.id))
            hitsAt1++;
        if (relevantInK > 0)
            hitsAtK++;
        let rr = 0;
